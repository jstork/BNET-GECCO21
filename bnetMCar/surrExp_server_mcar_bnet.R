...
while(length(fitness) <= 1000) {
  ...
  ##next Generation
  anns <- list(eliteFitAnn=eliteFitAnn,bestSurrAnn=bestSurrAnn,mutateElite=mutateElite)
...
write.csv(logData, file = paste("resMCar/bnet_mcar_surrExp_",seed,".csv",sep=""))
save(allResDiscounted, file= paste("resMCar/bnet_mcar_surrExp_res_",seed,".Rdata",sep=""))
write.csv(bestMethodCounter,file= paste("resMCar/bmc/bnet_mcar_surrExp_bm_",seed,".csv",sep=""))
write.csv(t,file= paste("resMCar/bmc/bnet_mcar_surrExp_bmt_",seed,".csv",sep=""))
