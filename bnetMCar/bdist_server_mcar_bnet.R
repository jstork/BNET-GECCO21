...
while(length(fitness) <= 1000) {
  ...
  ##next Generation
  anns <- list(eliteFitAnn=eliteFitAnn,eliteAbsBehAnn=eliteAbsBehAnn)
...
write.csv(logData, file = paste("resMCar/bnet_mcar_bdist_",seed,".csv",sep=""))
save(allResDiscounted, file= paste("resMCar/bnet_mcar_bdist_res_",seed,".Rdata",sep=""))
write.csv(bestMethodCounter,file= paste("resMCar/bmc/bnet_mcar_bdist_bm_",seed,".csv",sep=""))
write.csv(t,file= paste("resMCar/bmc/bnet_mcar_bdist_bmt_",seed,".csv",sep=""))
