...
while(length(fitness) <= 1000) {
  ...
  ##next Generation
  anns <- list(eliteFitAnn=eliteFitAnn,eliteCrossEntropyAnn=eliteCrossEntropyAnn,mutateElite=mutateElite)
...
write.csv(logData, file = paste("resMCar/bnet_mcar_crossExp_",seed,".csv",sep=""))
save(allResDiscounted, file= paste("resMCar/bnet_mcar_crossExp_res_",seed,".Rdata",sep=""))
write.csv(bestMethodCounter,file= paste("resMCar/bmc/bnet_mcar_crossExp_bm_",seed,".csv",sep=""))
write.csv(t,file= paste("resMCar/bmc/bnet_mcar_crossExp_bmt_",seed,".csv",sep=""))
