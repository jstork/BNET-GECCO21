...
while(length(fitness) <= 500) {
  ...
  #Testing
  if(i>1) {
    currRes <- lapply(anns[-5],computeFitnessStates,steps=200,instances=1,stochasticPolicy=FALSE,explorationRate = 0.1)
  ...
  ##next Generation
  anns <- list(eliteFitAnn=eliteFitAnn,
               eliteCrossEntropyAnn=eliteCrossEntropyAnn,
               eliteAbsBehAnn=eliteAbsBehAnn,
               bestSurrAnn=bestSurrAnn,
               mutateElite=mutateElite)
...
write.csv(logData, file = paste("resMCar/bnet_mcar_all_",seed,".csv",sep=""))
save(allResDiscounted, file= paste("resMCar/bnet_mcar_all_res_",seed,".Rdata",sep=""))
write.csv(bestMethodCounter,file= paste("resMCar/bmc/bnet_mcar_all_bm_",seed,".csv",sep=""))
write.csv(t,file= paste("resMCar/bmc/bnet_mcar_all_bmt_",seed,".csv",sep=""))
