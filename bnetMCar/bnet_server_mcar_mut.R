...
while(length(fitness) <= 5000) {
  ...
  critic <- trainCriticDummy(allResDiscounted,iteration=i)
  ...
  ##next Generation
  anns <- list(eliteFitAnn=eliteFitAnn,mutateElite=mutateElite)
...
write.csv(logData, file = paste("resMCar/bnet_mcar_mut_",seed,".csv",sep=""))
save(allResDiscounted, file= paste("resMCar/bnet_mcar_mut_res_",seed,".Rdata",sep=""))
write.csv(bestMethodCounter,file= paste("resMCar/bmc/bnet_mcar_mut_bm_",seed,".csv",sep=""))
write.csv(t,file= paste("resMCar/bmc/bnet_mcar_mut_bmt_",seed,".csv",sep=""))
