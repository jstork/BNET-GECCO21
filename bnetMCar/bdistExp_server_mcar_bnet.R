...
while(length(fitness) <= 1000) {
  ...
  #Testing
  if(i>1) {
    currRes <- lapply(anns[-3],computeFitnessStates,steps=200,instances=1,stochasticPolicy=FALSE,explorationRate = 0.1)
    exploreRes <- computeFitnessStates(anns[[3]],steps=200,instances=1,stochasticExploration=TRUE,explorationRate = 0.3)
  ...
  ##next Generation
  anns <- list(eliteFitAnn=eliteFitAnn,eliteAbsBehAnn=eliteAbsBehAnn,mutateElite=mutateElite)
...
write.csv(logData, file = paste("resMCar/bnet_mcar_bdistExp_",seed,".csv",sep=""))
save(allResDiscounted, file= paste("resMCar/bnet_mcar_bdist_resExp_",seed,".Rdata",sep=""))
write.csv(bestMethodCounter,file= paste("resMCar/bmc/bnet_mcar_bdistExp_bm_",seed,".csv",sep=""))
write.csv(t,file= paste("resMCar/bmc/bnet_mcar_bdistExp_bmt_",seed,".csv",sep=""))
