...
while(length(fitness) <= 500) {
  ...
  ##next Generation
  anns <- list(eliteFitAnn,eliteCrossEntropyAnn)
...
write.csv(logData, file = paste("resCartPole2/bnet_cartpole_cross2all_",seed,".csv",sep=""))
save(allResDiscounted, file= paste("resCartPole2/bnet_cartpole_cross2all_res_",seed,".Rdata",sep=""))
write.csv(bestMethodCounter,file= paste("resCartPole2/bmc/bnet_cartpole_cross2all_bm_",seed,".csv",sep=""))
write.csv(t,file= paste("resCartPole2/bmc/bnet_cartpole_cross2all_bmt_",seed,".csv",sep=""))
