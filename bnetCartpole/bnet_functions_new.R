...
extractExperienceDiscounted <- function(res,numOutputs) {
  ...
  for(i in 1:len) {
    ...
    #Select and sort
    selPosAction <- expActions[which(expRewards >= 0)]
    selPosRewards <- expRewards[which(expRewards >= 0)]
    ...
    selNegAction <- expActions[which(expRewards < 0)]
    expNegOutput<- rbind(expNegOutput,actionsToAvoid(selNegAction,numOutputs))
  }
...
###experience for maze function!! works only for robot maze
extractExperienceDiscreteMaze <- function(res,numOutputs=4,selSize= 1,threshold=0) {
  input <- res$input
  action <- res$action
  reward <- res$reward
  uniqueInputs <- unique(input)
  ...
  for(i in 1:len) {
    ...
    #aggregate Rewards
    agg <- data.frame(expActions,expRewards)
    agg <- aggregate(agg,by=(list(agg$expActions)),FUN=max)
    expActions <- agg$expActions
    expRewards <- agg$expRewards
    expRewardArchive <- c(expRewardArchive,expRewards)
    #Select and sort
    if (any(expRewards >= threshold)) {
      selPosAction <- expActions[which(expRewards >= threshold)]
      selPosRewards <- expRewards[which(expRewards >= threshold)]
      probs <- softmax(selPosRewards)
      expOutputNew<- c(rep(0,numOutputs)) ####number hardoced!!! TODO
      expOutputNew[selPosAction+1] <- probs
      expOutput<- rbind(expOutput,expOutputNew)
      expInput <- rbind(expInput,input[id[1],])
      expAdvantage <- c(expAdvantage,sum(selPosRewards))
    }
    if (any(expRewards < threshold)) {
      selNegAction <- expActions[which(expRewards < threshold)]
      selNegRewards <- expRewards[which(expRewards < threshold)]
      expOutput<- rbind(expOutput,actionsToAvoid(selNegAction,numOutputs))
      expInput <- rbind(expInput,input[id[1],])
      expAdvantage <- c(expAdvantage,sum(selNegRewards))
    }
  }
  return(list(input=expInput,output=expOutput,advantage=expAdvantage))
}

extractExperienceDiscreteMaze2 <- function(res,numOutputs=4,selSize= 1,threshold=0) {
  input <- res$input
  action <- res$action
  reward <- res$reward
  uniqueInputs <- unique(input)
...
trainCriticDummy <- function(allResDiscounted,iteration) {
  allResInputs <- lapply(allResDiscounted, FUN= '[[',2)
  ...
  learnValues  <- do.call('c',allValues)
  print("Fitting Critic")
  critic %>% fit(learnInputs, learnValues, epochs = 10,verbose = 0)
  scores = critic %>% evaluate(learnInputs, learnValues, verbose = 0)
  print(scores)
  return(critic)
}
...
computeCrossEntropyLossWeightedOld <- function(x,ann,inputBeh,outputBeh,reward) {
  if(!is.null(x)) {
    activeNodes <- ann$nodes[ann$nodes[,1]==1,]
    activeNodes[,-c(1:2,seq(3,ncol(activeNodes),2))] <- x
    ann$nodes[ann$nodes[,1]==1,] <- activeNodes
  ...
  logOutput[which(is.infinite(logOutput))] <- -10
...
crossEntropyFunction <- function(p, phat){
  x <- 0
  for (i in 1:length(p)){
    y <-  log(phat[i])
    if(is.infinite(y)) {
...
computeCrossEntropyLossWeighted <- function(x,ann,inputBeh,outputBeh,reward,onlyPos=TRUE) {
  if(!is.null(x)) {
    activeNodes <- ann$nodes[ann$nodes[,1]==1,]
    activeNodes[,-c(1:2,seq(3,ncol(activeNodes),2))] <- x
    ann$nodes[ann$nodes[,1]==1,] <- activeNodes
...
computeCrossEntropyLossWeightedAlt <- function(x,ann,inputBeh,outputBeh,reward,onlyPos=TRUE) {
  if(!is.null(x)) {
    activeNodes <- ann$nodes[ann$nodes[,1]==1,]
    activeNodes[,-c(1:2,seq(3,ncol(activeNodes),2))] <- x
    ann$nodes[ann$nodes[,1]==1,] <- activeNodes
  ...
  for (i in 1:nrow(output)) {
    res <- c(res,crossEntropyFunction(output[i,],outputBeh[i,]))
  }
  #print(res)
  if(onlyPos) {
    reward[which(reward < 0)] <- 0 
...
optimCrossEntropy<- function(anns,input,output,reward,iterations=1000,noElitist=2,numberChilds=20,mutationRate=0.05,plotFitness=TRUE,usePos=TRUE) {
  fitness=NULL
  numberChilds <- numberChilds/noElitist
  behFit <- unlist(lapply(anns,function(ann) computeCrossEntropyLossWeighted(x=NULL,ann,input,output,reward,onlyPos=usePos)))
  ...
  for(i in 1:iterations) {
    ...
    behFitOffspring <- unlist(lapply(offspring,function(ann) computeCrossEntropyLossWeighted(x=NULL,ann,input,output,reward,onlyPos=usePos)))
