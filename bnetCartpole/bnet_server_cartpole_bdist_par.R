...
#source("~/Documents/repos/s7pub/Stor20c.d/code/bnet_functions.R")
...
while(length(fitness) <= 500) {
  ...
  behFitAnns <- optimAbsBehaviour(behFitAnns,selectedExperienceInputs,selectedExperienceOutputs,selectedExperienceAdvantage,
                                  iterations=3000*trainingFactor,noElitist=2,numberChilds=20,mutationRate=0.01,plotFitness=FALSE)
  ...
  ##next Generation
  anns <- list(eliteFitAnn,eliteAbsBehAnn)
  ...
  for (id in 1:(popSize-1)) {
    anns[[id+1]]$id <- length(fitness)+id
  }
...
write.csv(logData, file = paste("resCartPole2/bnet_cartpole_bdistP_",seed,".csv",sep=""))
save(allResDiscounted, file= paste("resCartPole2/bnet_cartpole_bdistP_res_",seed,".Rdata",sep=""))
write.csv(bestMethodCounter,file= paste("resCartPole2/bmc/bnet_cartpole_bdistP_bm_",seed,".csv",sep=""))
write.csv(t,file= paste("resCartPole2/bmc/bnet_cartpole_bdistP_bmt_",seed,".csv",sep=""))
